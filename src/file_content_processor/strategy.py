...
class AmountHopsStrategy(Strategy):
    def process(self, raw_content):
        return (
            {"AMOUNT": int(round(1e3 * float(raw_content["AMOUNT"]), 0))}
            if raw_content
            else super().process(raw_content)
        )
...
class TimeStrategy(Strategy):
    def process(self, raw_content):
        return (
            {"TIME": datetime.timedelta(minutes=int(round(float(raw_content["TIME"]), 0)))}
            if raw_content
            else super().process(raw_content)
        )
...
class MashStepTimeStrategy(Strategy):
    def process(self, raw_content):
        return (
            {"STEP_TIME": datetime.timedelta(minutes=int(round(float(raw_content["STEP_TIME"]), 0)))}
            if raw_content
            else super().process(raw_content)
        )
...
class MashStepTempStrategy(Strategy):
    def process(self, raw_content):
        return (
            {"STEP_TEMP": int(round(float(raw_content["STEP_TEMP"]), 0))}
            if raw_content
            else super().process(raw_content)
        )
    ...
    def process(self, raw_content):
        if raw_content:
            ...
            content["WATER_GRAIN_RATIO"] = round(
                float(raw_content["MASH_STEPS"]["MASH_STEP"][0]["WATER_GRAIN_RATIO"].split()[0].replace(",", ".")),
                1,
            )
            content["INFUSE_VOLUME"] = round(
                float(raw_content["MASH_STEPS"]["MASH_STEP"][0]["DISPLAY_INFUSE_AMT"].split()[0]),
                2,
            )
            content["INFUSE_TEMP"] = round(
                float(raw_content["MASH_STEPS"]["MASH_STEP"][0]["INFUSE_TEMP"].split()[0]),
                1,
            )
            ...
            content["POST_BOIL_VOLUME"] = round(
                content["BOIL_VOLUME"]
                - (
                    content["EVAPORATION_PERCENTAGE"]
                    / 100
                    * content["BOIL_VOLUME"]
                    * int(round(float(raw_content["BOIL_TIME"]), 0))
                    / 60.0
                ),
                2,
            )
            content["KNOCKOUT_VOLUME"] = round(
                content["POST_BOIL_VOLUME"] * (1 - content["COOLING_SHRINKAGE_PERCENTAGE"] / 100),
                2,
            )
