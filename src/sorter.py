...
class MashStepsSorter(Sorter):
    _order = {'MashIn': 0, 'Beta': 1, 'Alpha': 2, 'MashOut': 3}

    def process(self, raw_content):
        return sorted(raw_content, key=lambda k: (self._order.get(self.__filter_non_alphabetic_characters(k['NAME']),
                                                                  self._get_max_order_value() + 1),
